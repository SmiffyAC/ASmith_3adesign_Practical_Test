...
def get_prices():
    ...
    token_lable = tk.Label(root, text="Getting a new access token...", fg="blue")
    token_lable.pack(pady=10)
    root.update()
    ...
    try:
        ...
        # Remove the token label
        token_lable.destroy()
    
    fetching_label = tk.Label(root, text="Fetching prices...", fg="blue")
    fetching_label.pack(pady=10)
    root.update()
    ...
    try:
        ...
        csv_header = ['Stock Code', 'Total Quantity', 'Quantity Per Set', 'ReelingFee', 'UnitPrice', 'ExtendedPrice', 'Error Title']
        ...
        with open(output_csv_filename, mode='w', newline='') as f:
            ...
            for stock_code, quantity in zip(stock_codes, quantities_needed):
                ...
                if response.status_code == 200:
                    ...
                    # Get the error title from the error message
                    error_title = response.json()['title']

                    # Write the error message to the CSV file
                    csv_writer.writerow([stock_code, set_quantity, quantity, 'N/A', 'N/A', 'N/A', error_title])
                    # Extract the original data from the DataFrame for the stock code
                    original_row = df[df['Stock Code'] == stock_code].fillna('').iloc[0].tolist()
                    # # Add the row to the list of error rows
                    # error_rows.append([stock_code, set_quantity, quantity, 'N/A', 'N/A', 'N/A', response.text])
                    
                    # Append the error title to the original data
                    original_row.append(error_title)

                    # Append the error message to the original data
                    original_row.append(response.text)

                    # Add the row with the original data and error to the list of error rows
                    error_rows.append(original_row)

            # Save the rows with errors to a separate CSV file
        if error_rows:
            original_headers = df.columns.tolist() + ['Error Title'] + ['Error Message']
            ...
            with open(error_csv_filename, mode='w', newline='') as error_file:
                error_writer = csv.writer(error_file)
                error_writer.writerow(original_headers)  # Write the original headers with 'Error'
                error_writer.writerows(error_rows)

        df = pd.read_csv(output_csv_filename)
        # Add together the reeling fees and ignore any rows with 'N/A' in the 'ReelingFee' column
        total_reeling_fee = df[df['ReelingFee'] != 'N/A']['ReelingFee'].sum()
        # Round the total reeling fee to 2 decimal places
        total_reeling_fee = round(total_reeling_fee, 2)
        print('Total Reeling Fee:', total_reeling_fee)
        ...
        # Add the total reeling fee abd total extended price together
        final_price = total_reeling_fee + total_extended_price
        # Print a showing the final price
        print('Final Price:', final_price)
        print('Price details saved to', output_csv_filename)

    except Exception as e:
        messagebox.showerror("Error", f"Failed to get prices: {e}")
        return
    
    finally:
        # Remove the fetching label
    ...
    error_rows = df[df['Error Title'] != 'N/A']
    ...
    # Add a label to display the total reeling fee
    reeling_fee_label = tk.Label(root, text=f"Total Reeling Fee: {total_reeling_fee}")
    reeling_fee_label.pack(pady=10)
    ...
    # Add a label to display the final price
    final_price_label = tk.Label(root, text=f"Final Price: {final_price}")
    final_price_label.pack(pady=10)
